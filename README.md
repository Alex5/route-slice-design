...
| **Т3** | **Импорт внутри `routes/` — только от предков.** Файл берёт из `routes/` только `-components/` и файлы сегментов-предков (`tasks.hooks.ts`, `wizard.context.tsx`). Вышестоящий сегмент не импортирует ничего из нижних, соседняя ветка — тоже. Файл маршрута не импортирует никто — к маршруту обращаются через `getRouteApi`. | check-architecture |

### Размещение и имена

| | Требование | Проверка |
|---|---|---|
| **Т4** | **Поднимать на втором использовании, не раньше.** Первое использование — на месте. Второе — к ближайшему общему предку: компонент в его `-components/`, хук в его `<предок>.hooks.ts`. В `shared/` попадает только то, что не знает домена (`ui`, `lib`), и сгенерированный контракт (`api`). Дублирование до этого момента — правильно, а не долг. | ревью |
| **Т5** | **Файлы сегмента называются по нему, папка для компонентов — одна.** В папке-сегменте лежат только файлы `<сегмент>.<роль>.ts(x)`: `projects.page.tsx`, `project-id.layout.tsx` (для `$projectId`), `tasks.loader.ts` — данные сегмента, `tasks.hooks.ts` — все хуки сегмента, `tasks.utils.ts`, `wizard.context.tsx`, `tasks.store.ts`. Страница или лэйаут есть в каждой папке-сегменте. `index.tsx` и `route.tsx` запрещены. Компоненты — в `-components/`, и это единственная папка с дефисом: отдельных папок для хуков и утилит нет, чтобы дерево показывало маршруты. | check-architecture |
...
2. Код нужен одному маршруту → рядом с ним: компонент в `-components/`, хук в `<сегмент>.hooks.ts`, функция в `<сегмент>.utils.ts`, лоадер в `<сегмент>.loader.ts`, данные на странице — хуком из `shared/api/hooks` (Т4, Т5, Т7, Т9).
