...
| **Т3** | **Импорт внутри `routes/` — только от предков.** Файл берёт из `routes/` только `_components/`, `_hooks/`, `_utils/` и файлы сегментов-предков (`tasks.loader.ts`, `wizard.context.tsx`). Вышестоящий сегмент не импортирует ничего из нижних, соседняя ветка — тоже. Файл маршрута не импортирует никто — к маршруту обращаются через `getRouteApi`. | check-architecture |

### Размещение и имена

| | Требование | Проверка |
|---|---|---|
| **Т4** | **Поднимать на втором использовании, не раньше.** Первое использование — на месте. Второе — к ближайшему общему предку: компонент в его `_components/`, хук в его `_hooks/`, функция в его `_utils/`. В `shared/` попадает только то, что не знает домена (`ui`, `lib`), и сгенерированный контракт (`api`). Дублирование до этого момента — правильно, а не долг. | ревью |
| **Т5** | **Файлы сегмента называются по нему, не-маршруты — в папках с `_`.** В папке-сегменте лежат только файлы `<сегмент>.<роль>.ts(x)`: `projects.page.tsx`, `project-id.layout.tsx` (для `$projectId`), `tasks.loader.ts` — данные сегмента, `wizard.context.tsx`, `tasks.store.ts`. Страница или лэйаут есть в каждой папке-сегменте. `index.tsx` и `route.tsx` запрещены. Компоненты — в `_components/`, хуки — в `_hooks/`, утилиты — в `_utils/`. Других папок с `_` нет. Подчёркивание отделяет не-маршруты от сегментов и ставит их вверх директории. | check-architecture |
...
| | Требование | Проверка |
|---|---|---|
| **Т7** | **Состояние — по происхождению.** С сервера — хук из `shared/api/hooks` (`useProject({ projectId })`), а `<сегмент>.loader.ts` заполняет кэш до рендера ([подробнее](#лоадер-маршрута--queryclientquery)). Разделяемое через ссылку — search-параметры URL. Общее для одного поддерева — контекст рядом с маршрутом. Локальное — `useState`. Архитектура называет происхождение, **инструмент выбирает стек**. | ревью |
...
| **Т9** | **Одна сетевая граница.** `shared/api` — единственное место, которое знает base URL, креды, форму ошибки, ключи запросов и хуки к ним, и единственное, где ошибка превращается в сообщение на экране. | check-architecture: запросы в маршрутах |
...
| Импорт | | Почему |
|---|---|---|
| `routes/projects/$projectId/tasks/_components/task-form/…` | ✅ | `tasks/` — предок |
| `routes/projects/_components/projects-table/…` | ✅ | `projects/` — предок |
| `routes/projects/$projectId/tasks/$taskId/_components/…` | ❌ Т3 | `$taskId/` — не предок, а сосед |
...
`projects/$projectId/_components/project-tabs` — вышестоящий сегмент ничего не
...
## Лоадер маршрута + queryClient.query

Серверные данные проходят три слоя, и у каждого своя задача:

```ts
// shared/api/hooks/tasks.ts — генерируется из OpenAPI (Т8, Т9)
...
```ts
// routes/…/$taskId/task-id.loader.ts — заполняет кэш до рендера
export async function loadTask({ context, params }) {
  try {
...
```tsx
// routes/…/$taskId/task-id.page.tsx — только подключает и читает
...
- **Страница знает только хук.** Ни `useSuspenseQuery`, ни ключей запросов в
  маршрутах нет — они в `shared/api/hooks`, там же, где их сгенерировал бы
  orval или kubb.
- **Данные есть до рендера.** Лоадер кладёт их в кэш, поэтому хук сразу
  возвращает данные, без `undefined` и спиннеров внутри страницы.
- **404 решает маршрут.** API бросает `ApiError(404)`, лоадер превращает её в
  `notFound()`; любая другая ошибка остаётся ошибкой.
- **Один кэш на всё.** Переход с карточки задачи на её редактирование не
  делает второй запрос, а инвалидация перерисовывает всех, кто читает хук.
- **Предзагрузка по наведению.** Роутер вызывает лоадер при наведении на
  ссылку (`defaultPreload: "intent"`).
- **Лоадер поднимается по Т4.** Задачу грузят и карточка, и редактирование,
  поэтому `task-id.loader.ts` лежит в их общем сегменте `$taskId`.

`staleTime: "static"` — лоадер берёт то, что уже есть в кэше, и загружает,
только если там пусто. Раньше для этого был `ensureQueryData`; в React Query
5.10x он объявлен устаревшим, и замена — ровно эта запись.

`QueryClient` создаётся в `app/providers` и попадает в лоадеры через контекст
роутера (`createRootRouteWithContext`), поэтому маршруты не импортируют `app`
(Т2). `defaultPreloadStaleTime: 0` отдаёт решение о свежести React Query, чтобы
роутер не держал поверх него свой кэш.
...
`*.page.tsx` — страница, `*.layout.tsx` — лэйаут, папки с подчёркиванием пропускаются.
...
2. Код нужен одному маршруту → рядом с ним: компонент в `_components/`, хук в `_hooks/`, функция в `_utils/`, лоадер в `<сегмент>.loader.ts`, данные на странице — хуком из `shared/api/hooks` (Т4, Т5, Т7, Т9).
3. Нужен второму маршруту → туда же, но у ближайшего общего предка (Т4).
...
src/
  ...
  routes/                  дерево URL — папки здесь и есть адресная строка
    _components/           обвязка на каждом URL (шапка)
